...
public interface AdmMenusRepository extends JpaRepository<AdmMenu, Integer> {
    @Query(value = "SELECT DISTINCT d.* FROM ((adm_user a LEFT JOIN adm_role_user b ON a.id_adm_user = b.id_adm_user) LEFT JOIN adm_role_menu c ON b.id_adm_role = c.id_adm_role) LEFT JOIN adm_menu d ON c.id_adm_menu = d.id_adm_menu WHERE a.name = :username AND d.id_adm_menu IS NOT NULL", nativeQuery = true)
