...
public class UIWrapperService {
	...
	public void swapCiTypeLayerPosition(int layerId, int targetLayerId) {
		...
		updateEnumCodes(Lists.newArrayList(BeanMapUtils.convertBeanToMap(enumCode),
				BeanMapUtils.convertBeanToMap(targetEnumCode)));
	}
	...
	public void swapCiTypeAttributePosition(int attributeId, int targetAttributeId) {
		...
		updateCiTypeAttributes(
				BeanMapUtils.convertBeansToMaps(Lists.newArrayList(updateSourceAttribute, updateTargetAttribute)));
	}
	...
	public List<CatCodeDto> getCiTypesGroupByLayers(boolean withAttributes, String status) {
		return getCiTypesInGroups(uiProperties.getEnumCategoryCiTypeLayer(), withAttributes, status,
				CiTypeDto::getLayerId);
	}

	public List<CatCodeDto> getCiTypesGroupByCatalogs(boolean withAttributes, String status) {
		return getCiTypesInGroups(uiProperties.getEnumCategoryCiTypeCatalog(), withAttributes, status,
				CiTypeDto::getCatalogId);
	}
	...
	public List<CiTypeAttrDto> getCiTypeReferenceBy(Integer ciTypeId) {
		QueryRequest queryObject = new QueryRequest().addEqualsFilter("referenceId", ciTypeId)
	...
	public List<CiTypeAttrDto> getCiTypeReferenceTo(Integer ciTypeId) {
		QueryRequest queryObject = new QueryRequest().addEqualsFilter(CONSTANT_CI_TYPE_ID, ciTypeId)
				.addInFilter(CONSTANT_INPUT_TYPE, Arrays.asList("ref", "multiRef"));
		queryObject.addReferenceResource(CONSTANT_CI_TYPE);
		return queryCiTypeAttributes(queryObject);
	}
	...
	private List<CatCodeDto> getCiTypesInGroups(String categoryName, boolean withAttributes, String status,
			Function<CiTypeDto, Object> parentMapperOfElement) {
	...
	public List<CatCodeDto> getCiTypeStatusOptions(int ciTypeId) {
		List<CiTypeAttrDto> statusCiTypeAttributes = queryCiTypeAttributes(
				defaultQueryObject(CONSTANT_CI_TYPE_ID, ciTypeId).addEqualsFilter("propertyName",
						uiProperties.getStatusAttributeName()));
	...
	public List<CatCodeDto> getEnumCodesByCategoryId(Integer categoryId) {
		QueryResponse<CatCodeDto> response = staticDtoService.query(CatCodeDto.class,
				defaultQueryObject(CONSTANT_CAT_ID, categoryId).ascendingSortBy(CONSTANT_SEQ_NO));
		return response != null ? response.getContents() : null;
	}
	...
	public List<CiTypeDto> getAllCiTypes(boolean withAttributes, String status) {
		...
		if (status != null) {
			request.getFilters()
					.add(new Filter("status", FilterOperator.In.getCode(), Arrays.asList(status.split(","))));
		}
		...
		if (withAttributes) {
			...
			if (status != null) {
				request.getFilters().add(
						new Filter("attributes.status", FilterOperator.In.getCode(), Arrays.asList(status.split(","))));
			}
		}
	...
	public List<RoleDto> getRolesByUsername(String username) {
		QueryRequest queryObject = defaultQueryObject().addReferenceResource("roleUsers")
				.addReferenceResource("roleUsers.user").addEqualsFilter("roleUsers.user.username", username);
	...
	public List<UserDto> getUsersByRoleId(int roleId) {
		QueryRequest queryObject = defaultQueryObject().addReferenceResource("roleUsers")
				.addReferenceResource("roleUsers.role").addEqualsFilter("roleUsers.role.roleId", roleId);
	...
	public List<RoleCiTypeDto> getRoleCiTypeByRoleId(int roleId) {
		return queryRoleCiTypes(
				defaultQueryObject().addReferenceResource("roleCiTypeCtrlAttrs").addEqualsFilter("roleId", roleId));
	}

	public List<RoleCiTypeDto> getRoleCiTypeByUsername(String username) {
		return queryRoleCiTypes(defaultQueryObject().addReferenceResource("role").addReferenceResource("role.roleUsers")
				.addReferenceResource("role.roleUsers.user").addReferenceResource("roleCiTypeCtrlAttrs")
	...
	public RoleCiTypeDto getRoleCiTypeByRoleIdAndCiTypeId(int roleId, int ciTypeId) {
		QueryRequest queryObject = defaultQueryObject().addEqualsFilter("roleId", roleId)
				.addEqualsFilter(CONSTANT_CI_TYPE_ID, ciTypeId);
		QueryResponse<RoleCiTypeDto> response = staticDtoService.query(RoleCiTypeDto.class, queryObject);
		if (response != null && !response.getContents().isEmpty()) {
			return response.getContents().get(0);
		}
	...
	public List<RoleCiTypeCtrlAttrDto> queryRoleCiTypeCtrlAttributes(QueryRequest queryObject) {
		QueryResponse<RoleCiTypeCtrlAttrDto> response = staticDtoService.query(RoleCiTypeCtrlAttrDto.class,
				queryObject);
		return response != null ? response.getContents() : null;
	}

	public List<RoleCiTypeCtrlAttrDto> getRoleCiTypeCtrlAttributesByRoleCiTypeId(int roleCiTypeId) {
		return queryRoleCiTypeCtrlAttributes(
				defaultQueryObject().addReferenceResource("conditions").addEqualsFilter("roleCiTypeId", roleCiTypeId));
	}
	...
	public RoleCiTypeCtrlAttrConditionDto getRoleCiTypeCtrlAttributeConditionById(Integer conditionId) {
		...
		QueryResponse<RoleCiTypeCtrlAttrConditionDto> response = staticDtoService
				.query(RoleCiTypeCtrlAttrConditionDto.class, queryObject);
		if (response != null && !response.getContents().isEmpty()) {
			return response.getContents().get(0);
		}
		return null;
	...
	public List<RoleCiTypeCtrlAttrConditionDto> createRoleCiTypeCtrlAttrConditions(
			RoleCiTypeCtrlAttrConditionDto... roleCiTypeCtrlAttrConditions) {
		return staticDtoService.create(RoleCiTypeCtrlAttrConditionDto.class,
				Arrays.asList(roleCiTypeCtrlAttrConditions));
	}

	public RoleCiTypeCtrlAttrConditionDto createRoleCiTypeCtrlAttrCondition(
			RoleCiTypeCtrlAttrConditionDto roleCiTypeCtrlAttrCondition) {
		List<RoleCiTypeCtrlAttrConditionDto> roleCiTypeCtrlAttrConditions = createRoleCiTypeCtrlAttrConditions(
				roleCiTypeCtrlAttrCondition);
	...
	public List<RoleCiTypeCtrlAttrConditionDto> updateRoleCiTypeCtrlAttrConditions(
			List<Map<String, Object>> roleCiTypeCtrlAttrConditions) {
	...
	public List<CiTypeAttrDto> getCiTypeAccessControlAttributesByCiTypeId(Integer ciTypeId) {
		QueryRequest request = defaultQueryObject(CONSTANT_CI_TYPE_ID, ciTypeId)
				.addEqualsFilter("isAccessControlled", 1).ascendingSortBy("displaySeqNo");
	...
	public List<CatCodeDto> getEnumCodesByIds(List<Integer> ids) {
		QueryResponse<CatCodeDto> queryResult = staticDtoService.query(CatCodeDto.class,
				defaultQueryObject().addInFilter("codeId", ids));
		return queryResult != null ? queryResult.getContents() : null;
	}
	...
	public RoleCiTypeCtrlAttrDto updateRoleCiTypeCtrlAttribute(RoleCiTypeCtrlAttrDto roleCiTypeCtrlAttr) {
		List<RoleCiTypeCtrlAttrDto> roleCiTypeCtrlAttrs = updateRoleCiTypeCtrlAttributes(
				Lists.newArrayList(BeanMapUtils.convertBeanToMap(roleCiTypeCtrlAttr)));
		if (isEmpty(roleCiTypeCtrlAttrs))
	...
	public List<RoleCiTypeCtrlAttrConditionDto> updateRoleCiTypeCtrlAttrConditions(
			RoleCiTypeCtrlAttrConditionDto... roleCiTypeCtrlAttrConditions) {
		return staticDtoService.update(RoleCiTypeCtrlAttrConditionDto.class,
				Lists.newArrayList(BeanMapUtils.convertBeanToMap(roleCiTypeCtrlAttrConditions)));
	}

	public List<RoleCiTypeDto> updateRoleCiTypes(RoleCiTypeDto... roleCiTypes) {
		return staticDtoService.update(RoleCiTypeDto.class,
				BeanMapUtils.convertBeansToMaps(Arrays.asList(roleCiTypes)));
	}
	...
	public void recursiveGetChildrenData(Integer ciTypeId, List<Integer> limitedCiTypes,
			List<ResourceTreeDto> resourceTrees, Map<String, Object> inputFilters) {
		...
		for (int i = 0; i < ciDatas.size(); i++) {
			...
			if (childrenCiTypeRelativeAttributes.size() != 0) {
				recursiveGetChildrenDataByRelativeAttributes(childrenCiTypeRelativeAttributes, limitedCiTypes,
						ciDataMap.get("guid").toString(), resourceTrees.get(i).getChildren());
				continue;
			}

			List<CiTypeAttrDto> runningCiTypeRelativeAttributes = findChildrenCiTypeRelativeAttributes(ciTypeId,
					uiProperties.getReferenceCodeOfRunning());
			if (runningCiTypeRelativeAttributes.size() != 0) {
				recursiveGetChildrenDataByRelativeAttributes(runningCiTypeRelativeAttributes, limitedCiTypes,
						ciDataMap.get("guid").toString(), resourceTrees.get(i).getChildren());
			}
		}
	}
	...
	private List<ResourceTreeDto> getDataTreesByCiTypeIdAndGuid(int ciTypeId, List<String> guids) {
		...
		for (int i = 0; i < ciDatas.size(); i++) {
			...
			for (CiTypeAttrDto childrenCiTypeRelativeAttribute : childrenCiTypeRelativeAttributes) {
				...
				recursiveGetChildrenData(childrenCiTypeRelativeAttribute.getCiTypeId(), sameLayerCiTypes,
						resourceTrees.get(i).getChildren(), filter);
			}
		}
		return resourceTrees;
	}
	...
	private List<CiTypeAttrDto> findChildrenCiTypeRelativeAttributes(Integer ciTypeId, String referenceCode) {
		...
		for (CiTypeAttrDto attrDto : referenceByList) {
			if (attrDto.getReferenceType() != null
					&& baseKeyInfoService.getCode(attrDto.getReferenceType()).getCode() != null
	...
	private void recursiveGetChildrenDataByRelativeAttributes(List<CiTypeAttrDto> childrenCiTypeRelativeAttributes,
			List<Integer> limitedCiTypes, String guid, List<ResourceTreeDto> children) {
		for (int j = 0; j < childrenCiTypeRelativeAttributes.size(); j++) {
			...
			recursiveGetChildrenData(childrenCiTypeRelativeAttributes.get(j).getCiTypeId(), limitedCiTypes, children,
					filter);
		}
	}

	private void recursiveGetChildrenDataByRelativeAttributes(List<CiTypeAttrDto> childrenCiTypeRelativeAttributes,
			String stateEnumCode, String guid, List<ResourceTreeDto> children) {
		if (childrenCiTypeRelativeAttributes.size() != 0) {
			for (CiTypeAttrDto childrenCiTypeRelativeAttribute : childrenCiTypeRelativeAttributes) {
				Map<String, Object> filter = new HashMap<>();
				...
				recursiveGetChildrenDataFilterState(childrenCiTypeRelativeAttribute.getCiTypeId(),
						stateEnumCatOfChildren, stateEnumCode, children, filter);
			}
		}
	}
	...
	public List<ZoneLinkDto> getAllZoneLinkDesignGroupByIdcDesign() {
		...
		List<CiData> idcDesignData = queryCiData(uiProperties.getCiTypeIdOfIdcDesign(), defaultQueryObject())
				.getContents();
		...
		for (CiData idcDesign : idcDesignData) {
			...
			List<CiData> zoneDesignData = queryCiData(uiProperties.getCiTypeIdOfZoneDesign(),
					defaultQueryObject().addEqualsFilter("data_center_design", idcDesignGuid)).getContents();
			...
			if (zoneDesignList.size() != 0) {
				List<CiData> zoneDesignLinkData = queryCiData(uiProperties.getCiTypeIdOfZoneLinkDesign(),
						defaultQueryObject().addInFilter("network_zone_design_1", zoneDesignList)
								.addInFilter("network_zone_design_2", zoneDesignList).setFiltersRelationship("or"))
										.getContents();
	...
	public List<ZoneLinkDto> getAllZoneLinkGroupByIdc() {
		...
		for (CiData idc : idcData) {
			...
			List<CiData> zoneData = queryCiData(uiProperties.getCiTypeIdOfZone(),
					defaultQueryObject().addEqualsFilter("data_center", idcGuid)).getContents();
			...
			if (zoneList.size() != 0) {
				List<CiData> zoneLinkData = queryCiData(uiProperties.getCiTypeIdOfZoneLink(),
						defaultQueryObject().addInFilter("network_zone_1", zoneList)
								.addInFilter("network_zone_2", zoneList).setFiltersRelationship("or")).getContents();
	...
	public List<ResourceTreeDto> getAllDesignTreesFromSystemDesign(String systemDesignGuid) {
		...
		List<CiTypeAttrDto> attr = getCiTypeAttributesByCiTypeIdAndPropertyName(systemDesignCiTypeId,
				uiProperties.getPropertyNameOfState());
		if (attr.size() == 0) {
			return null;
		}
	...
	private boolean checkCiTypeAttributes(List<CiTypeAttrDto> ciTypeAttributes, int stateEnumCat, String stateEnumCode,
			Map ciDataMap) {
		boolean delconFlag = true;
		for (CiTypeAttrDto ciTypeAttribute : ciTypeAttributes) {
			if (ciTypeAttribute.getInputType().equals(CONSTANT_SELECT)
					&& ciTypeAttribute.getReferenceId() == stateEnumCat) {
				delconFlag = false;
				try {
					Map catCodeData = (Map) ciDataMap.get(ciTypeAttribute.getPropertyName());
					String fixedDate = ciDataMap.get("fixed_date").toString();
					if (stateEnumCode.equalsIgnoreCase(catCodeData.get("code").toString()) && fixedDate != null
							&& fixedDate.length() > 0) {
						delconFlag = true;
						break;
					}
	...
	public List<CatCodeDto> getEnumCodesByGroupId(Integer groupId) {
		QueryResponse<CatCodeDto> queryResult = staticDtoService.query(CatCodeDto.class,
				defaultQueryObject("groupCodeId", groupId).ascendingSortBy(CONSTANT_SEQ_NO));
		return queryResult.getContents();
	}

	private List<Map<String, Object>> getAllCiDataOfRootCi(int rootCiTypeId, int envEnumCat, String envEnumCode,
			int filterCiTypeId, List<String> guid, String routine) {
		...
		if (envEnumCode != null && enumPorpertyNameOfEnv != null) {
			Filter rootCifilter = new Filter("root$" + enumPorpertyNameOfEnv, "eq",
		...
		IntegrationQueryDto childQueryDto = travelRoutine(routineItems, filterCiTypeId, rootDto, 1, "", envEnumCat,
				envEnumCode);
	...
	private String getEnumPropertyNameByCiTypeId(int ciTypeId, int enumCat) {
		...
		for (int j = 0; j < ciTypeAttributes.size(); j++) {
			if (ciTypeAttributes.get(j).getInputType().equals(CONSTANT_SELECT)
					&& ciTypeAttributes.get(j).getReferenceId() == enumCat) {
	...
	private IntegrationQueryDto travelRoutine(List<CiRoutineItem> routines, int filterCiTypeId,
			AdhocIntegrationQueryDto rootDto, int position, String key, int envEnumCat, String envEnumCode) {
		...
		IntegrationQueryDto childDto = travelRoutine(routines, filterCiTypeId, rootDto, ++position, "", envEnumCat,
				envEnumCode);
		if (childDto == null) {
			...
			dto.setAttrs(Arrays.asList(getAttrIdByCiTypeId(item.getCiTypeId(), "guid"),
					getAttrIdByCiTypeId(item.getCiTypeId(), "r_guid")));
	...
	public List<ResourceTreeDto> getAllDeployTreesFromSubSys(String envCode, String systemDesignGuid) {
		...
		String routine = null;
		List<CatCodeDto> codeOfRoutines = getEnumCodeByCodeAndCategoryName(uiProperties.getCodeOfDeployDetail(),
				uiProperties.getCatNameOfQueryDeployDesign());
		if (codeOfRoutines.size() > 0) {
		...
		List<Map<String, Object>> ciDatas = getAllCiDataOfRootCi(subsysCiTypeId, envEnumCat, envCode,
				systemDesignCiTypeId, guid, routine);

		List<CiTypeAttrDto> attrOfSubsys = getCiTypeAttributesByCiTypeIdAndPropertyName(subsysCiTypeId,
				uiProperties.getPropertyNameOfState());
		...
		for (int i = 0; i < ciDatas.size(); i++) {
			...
			List<ResourceTreeDto> resourceTrees = new ArrayList<>();
			recursiveGetChildrenDataFilterState(subsysCiTypeId, stateEnumCatOfSubsys, stateEnumCode, resourceTrees,
					filter);
	...
	public List<ResourceTreeDto> getApplicationDeploymentDesignDataTreeBySystemDesignGuidAndEnvCode(
			String systemDesignGuid, Integer envCodeId) {
		...
		List<Integer> limitedCiTypeIdsOfGetInstanceData = Lists.newArrayList(uiProperties.getCiTypeIdOfSystemDesign(),
				uiProperties.getCiTypeIdOfSubsystemDesign(), uiProperties.getCiTypeIdOfSubsys(),
				uiProperties.getCiTypeIdOfUnit(), uiProperties.getCiTypeIdOfInstance());
		...
		getBottomChildrenDataByBottomCiTypeId(uiProperties.getCiTypeIdOfSystemDesign(),
				uiProperties.getCiTypeIdOfInstance(), instanceData, limitedCiTypeIdsOfGetInstanceData,
				systemDesignfilter, subsystemfilter);

		List<CiTypeAttrDto> relateCiAttrDtoList = getRefToAttrByCiTypeIdAndRefName(uiProperties.getCiTypeIdOfInstance(),
				uiProperties.getReferenceCodeOfRunning());
	...
	private void getBottomChildrenDataByBottomCiTypeId(Integer ciTypeId, Integer bottomCiTypeId,
			List<ResourceTreeDto> bottomChildrenData, List<Integer> limitedCiTypeIds, Map<String, Object> inputFilters,
		...
		for (CiData ciData : ciDatas) {
			...
			if (childrenCiTypeRelativeAttributes.size() != 0 && ciDataMap.get("guid") != null) {
				findBelongCi = getBottomChildrenDataByRelativeAttributes(childrenCiTypeRelativeAttributes,
						limitedCiTypeIds, ciDataMap.get("guid").toString(), bottomChildrenData, bottomCiTypeId,
						subsystemFilters);
			}
			...
			if (!findBelongCi) {
				...
				if (realizeCiTypeRelativeAttributes.size() != 0 && ciDataMap.get("guid") != null) {
					getBottomChildrenDataByRealizeAttributes(realizeCiTypeRelativeAttributes, limitedCiTypeIds,
							ciDataMap.get("guid").toString(), bottomChildrenData, bottomCiTypeId, subsystemFilters);
				}
			}
		}
	}

	private void getBottomChildrenDataByRealizeAttributes(List<CiTypeAttrDto> realizeCiTypeRelativeAttributes,
			List<Integer> limitedCiTypeIds, String guid, List<ResourceTreeDto> bottomChildrenData,
			Integer bottomCiTypeId, Map<String, Object> subsystemFilters) {
		for (CiTypeAttrDto realizeCiTypeRelativeAttribute : realizeCiTypeRelativeAttributes) {
			...
			getBottomChildrenDataByBottomCiTypeId(realizeCiTypeRelativeAttribute.getCiTypeId(), bottomCiTypeId,
					bottomChildrenData, limitedCiTypeIds, filter, subsystemFilters);
		}
	}
	...
	private List<CiTypeAttrDto> findRealizeCiAttributesByCiTypeId(Integer ciTypeId) {
		...
		for (CiTypeAttrDto attrDto : referenceByList) {
			if (attrDto.getReferenceType() != null
					&& baseKeyInfoService.getCode(attrDto.getReferenceType()).getCode() != null
					&& baseKeyInfoService.getCode(attrDto.getReferenceType()).getCode()
							.equals(uiProperties.getReferenceCodeOfRealize())) {
				ChildrenCiTypeRelativeAttributes.add(attrDto);
			}
		}
		return ChildrenCiTypeRelativeAttributes;
	...
	private boolean getBottomChildrenDataByRelativeAttributes(List<CiTypeAttrDto> childrenCiTypeRelativeAttributes,
			List<Integer> limitedCiTypeIds, String guid, List<ResourceTreeDto> bottomChildrenData,
			Integer bottomCiTypeId, Map<String, Object> subsystemFilters) {
		for (CiTypeAttrDto childrenCiTypeRelativeAttribute : childrenCiTypeRelativeAttributes) {
			Map<String, Object> filter = new HashMap<>();
			...
			getBottomChildrenDataByBottomCiTypeId(childrenCiTypeRelativeAttribute.getCiTypeId(), bottomCiTypeId,
					bottomChildrenData, limitedCiTypeIds, filter, subsystemFilters);
			return true;
		}
		return false;
	...
	public List<ResourceTreeDto> getApplicationFrameworkDesignDataTreeBySystemDesignGuid(String systemDesignGuid) {
		...
		getBottomChildrenDataByBottomCiTypeId(uiProperties.getCiTypeIdOfSystemDesign(),
				uiProperties.getCiTypeIdOfUnitDesign(), unitDesignDatas,
				getSameCiTypesByCiTypeId(uiProperties.getCiTypeIdOfSystemDesign()), inputFilter, new HashMap<>());
	...
	public void initModel() {
		deleteByTableName(ADM_CI_TYPE_ATTR);
		deleteByTableName(ADM_CI_TYPE);
	}
