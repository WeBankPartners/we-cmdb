  ...
  methods: {
      ...
      const myCiGetter = async (exprData, isBackref, guids) => {
        ...
        if (cacheData[id]) {
          ...
          const { statusCode, data } = await queryCiData({
            id: this.allCITypes[exprData.ci].ciTypeId,
            queryObject: {
    ...
    exprSegParse (expr) {
      ...
      if (segs) {
        ...
        ret.filters = this.exprFilterParse(ret.filters)
        return ret
      }
      // throw 'invalid expression: ' + expr
    },
    exprParse (expr) {
      let splitRes = this.exprSplit(expr, this.exprOpFinder(expr))
      splitRes.forEach(el => {
        if (el.type === 'expr') {
          el.data = this.exprSegParse(el.value)
        }
      })
      return splitRes
    },
