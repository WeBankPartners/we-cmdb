...
export default {
  ...
  methods: {
    ...
    showRuleOptions(ruleIndex, attrIndex) {
      this.options = [];
      this.optionsDisplay = true;
      const isAttrNode = attrIndex
        ? !!JSON.parse(this.autoFillArray[ruleIndex].value)[attrIndex].parentRs
        : false;
      ...
      if (
        ...
        const ciTypeId = JSON.parse(this.autoFillArray[ruleIndex].value)[
          attrIndex
        ].ciTypeId;
        this.getRefData(ruleIndex, attrIndex, ciTypeId);
      } else if (
        (node.parentRs &&
          this.ciTypeAttrsObj[node.parentRs.attrId].inputType === "select") ||
    ...
    async getRefData(ruleIndex, attrIndex, ciTypeId) {
      ...
      const promiseArray = [
        getRefCiTypeFrom(ciTypeId),
        getCiTypeAttr(ciTypeId)
      ];
      ...
      if (refFroms.statusCode === "OK" && ciAttrs.statusCode === "OK") {
        ...
        this.options = this.options.concat(
          ciAttrs.data.map(_ => {
            ...
            const ciTypeName = isRef
              ? this.ciTypesObj[_.referenceId].name
              : this.ciTypesObj[_.ciTypeId].name;
            const attrName = this.ciTypeAttrsObj[_.ciTypeAttrId].name;
            const nodeName = isRef
              ? `->(${ciTypeName})${attrName}`
              : `->${attrName}`;
            const nodeObj = {
      ...
      if (!attrIndex) {
        ...
        if (attrIndex === "0") {
          ...
          if (
            ...
            this.autoFillArray[+ruleIndex - 1].value += this.autoFillArray[
              ruleIndex
            ].value;
            this.autoFillArray.splice(ruleIndex, 1);
          }
    ...
    async showFilterModal(ruleIndex, attrIndex) {
      this.filterCiTypeId = JSON.parse(this.autoFillArray[ruleIndex].value)[
        attrIndex
      ].ciTypeId;
      const filters =
        JSON.parse(this.autoFillArray[ruleIndex].value)[attrIndex].filters ||
        [];
    ...
    addNode(ruleIndex, attrIndex, nodeObj) {
      ...
      this.autoFillArray[ruleIndex].value = JSON.stringify(ruleArr);
      const inputType = this.ciTypeAttrsObj[
    ...
    renderExpression(val, i) {
      ...
      let result = JSON.parse(val.value).map((_, attrIndex) => {
        ...
        if (attrIndex === JSON.parse(val.value).length - 1) {
          const lastInputType = JSON.parse(val.value)[attrIndex].parentRs
            ? this.ciTypeAttrsObj[
                JSON.parse(val.value)[attrIndex].parentRs.attrId
              ].inputType
            : "";
          if (
            lastInputType === "ref" ||
            lastInputType === "multiRef" ||
            !lastInputType
          ) {
            isLegal = false;
          } else if (
            lastInputType === "select" ||
            lastInputType === "multiSelect"
          ) {
          ...
          filterNode += _.filters
            .map((filter, filterIndex) => {
              const filterValue = Array.isArray(filter.value)
                ? `[${filter.value.join(",")}]`
                : filter.value;
              return `${filterIndex > 0 ? " | " : ""}${filter.name} ${
                filter.operator
              } ${filterValue}`;
            })
            .join("");
        ...
        if (!_.parentRs) {
          return `${ciTypeName}${filterNode}`.split("").map(_ => {
            const classList = {
              "auto-fill-span": true,
              "auto-fill-hover":
                this.hoverAttr === attrIndex + "" && this.hoverSpan === i + "",
              "auto-fill-current-node":
                this.currentRule === i + "" &&
                this.currentAttr === attrIndex + "",
          ...
          if (
            ...
            return ` ${ref}(${ciTypeName})${attrName}${filterNode}`
              .split("")
              .map(_ => {
                const classList = {
                  "auto-fill-span": true,
                  "auto-fill-hover":
                    this.hoverAttr === attrIndex + "" &&
                    this.hoverSpan === i + "",
                  "auto-fill-current-node":
                    this.currentRule === i + "" &&
                    this.currentAttr === attrIndex + "",
            ...
            return ` ${ref}${attrName}${enumCode}${filterNode}`
              .split("")
              .map(_ => {
                const classList = {
                  "auto-fill-span": true,
      ...
      return [
        <span
          class={`auto-fill-span auto-fill-braces${
            this.hoverSpan === i + "" ? " contains" : ""
          }`}
          index={i}
        >
          {"{ "}
      ...
      if (this.activeDelimiterIndex === i + "") {
        ...
        result = [
          ...
          val.split("").map(_ => {
            return (
              <span
                class={`auto-fill-span${
                  this.hoverSpan === i + "" ? " hover" : ""
                }`}
                index={i}
              >
                {_}
    ...
    renderSpecialDelimiter(value, i) {
      return [
        ...
        value.split("").map(_ => {
          return (
            <span
              class={`auto-fill-span auto-fill-special-delimiter${
                this.hoverSpan === i + "" ? " hover" : ""
              }`}
              index={i}
            >
    ...
    confirmDelimiter(i) {
      if (this.activeDelimiterValue === "") {
        ...
        if (
          this.autoFillArray[i + 1] &&
          this.autoFillArray[i + 1].type === "delimiter"
        ) {
    ...
    renderAddRule() {
      return [
        ...
        !this.autoFillArray.length && (
          <span class="auto-fill-add auto-fill-placeholder">
    ...
    handleInput() {
      this.$nextTick(() => {
        const value = this.autoFillArray.length
          ? JSON.stringify(this.autoFillArray)
          : "";
        let isLegal = true;
        this.autoFillArray.forEach(_ => {
          if (_.type === "rule") {
            ...
            const lastAttrId = ruleArray[ruleArray.length - 1].parentRs
              ? ruleArray[ruleArray.length - 1].parentRs.attrId
              : 0;
            if (lastNode.parentRs) {
              if (
                this.ciTypeAttrsObj[lastAttrId].inputType === "ref" ||
                this.ciTypeAttrsObj[lastAttrId].inputType === "multiRef"
              ) {
                isLegal = false;
              } else if (
                this.ciTypeAttrsObj[lastAttrId].inputType === "select" ||
                this.ciTypeAttrsObj[lastAttrId].inputType === "multiSelect"
              ) {
